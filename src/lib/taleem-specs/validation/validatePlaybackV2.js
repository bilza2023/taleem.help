...
// /src/lib/taleem-specs/validation/validatePlaybackV2.js
...
function validateShowAtRanges(slides, errors) {
	...
	slides.forEach((slide, si) => {
		...
		slide.data.forEach((item, di) => {
			...
			if (
				item.showAt < slide.start ||
				item.showAt >= slide.end
			) {
	...
	slides.forEach((slide, si) => {
		...
		slide.data.forEach((line, li) => {
			...
			if (typeof line.showAt === "number") {
				...
				if (
					line.showAt < slide.start ||
					line.showAt >= slide.end
				) {
